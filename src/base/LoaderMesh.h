...
struct Mesh {
	...
	Eigen::Matrix<int32_t, -1, -1> F;
};

inline static void load_mesh(std::string filename, Mesh &mesh, bool verbose = false) {
	if (filename.find(".obj") != std::string::npos) { 
		...
		std::shared_ptr<tinyply::PlyData> vertices, colors, elements;
		...
		const size_t n_colors_bytes = colors->buffer.size_bytes();
		mesh.C.resize(3, colors->count);
		std::memcpy(mesh.C.data(), colors->buffer.get(), n_colors_bytes);
