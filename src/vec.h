...
template<class T>
__device__ inline T sqr(const T& x)
{ return x*x; }
...
struct Vec
{
   ...
   __device__ Vec<N,T>(void)
   {}

   __device__ explicit Vec<N,T>(T value_for_all)
   { for(unsigned int i=0; i<N; ++i) v[i]=value_for_all; }

   template<class S>
   __device__ explicit Vec<N,T>(const S *source)
   { for(unsigned int i=0; i<N; ++i) v[i]=(T)source[i]; }

   template <class S>
   __device__ explicit Vec<N,T>(const Vec<N,S>& source)
   { for(unsigned int i=0; i<N; ++i) v[i]=(T)source[i]; }

   __device__ Vec<N,T>(T v0, T v1)
   {
      assert(N==2);
      v[0]=v0; v[1]=v1;
   }

   __device__ Vec<N,T>(T v0, T v1, T v2)
   {
      assert(N==3);
      v[0]=v0; v[1]=v1; v[2]=v2;
   }

   __device__ Vec<N,T>(T v0, T v1, T v2, T v3)
   {
      assert(N==4);
      v[0]=v0; v[1]=v1; v[2]=v2; v[3]=v3;
   }

   __device__ Vec<N,T>(T v0, T v1, T v2, T v3, T v4)
   {
      assert(N==5);
      v[0]=v0; v[1]=v1; v[2]=v2; v[3]=v3; v[4]=v4;
   }

     __device__ Vec<N,T>(T v0, T v1, T v2, T v3, T v4, T v5)
   {
      assert(N==6);
   ...
   __device__ bool nonzero(void) const
   {
      for(unsigned int i=0; i<N; ++i) if(v[i]) return true;
      return false;
   }

   __device__ Vec<N,T> operator+=(const Vec<N,T> &w)
   {
      for(unsigned int i=0; i<N; ++i) v[i]+=w[i];
      return *this;
   }

   __device__ Vec<N,T> operator+(const Vec<N,T> &w) const
   {
      Vec<N,T> sum(*this);
      sum+=w;
      return sum;
   ...
   __device__ Vec<N,T> operator-(void) const // unary minus
   {
      Vec<N,T> negative;
   ...
   __device__ Vec<N,T> operator-(const Vec<N,T> &w) const // (binary) subtraction
   {
      Vec<N,T> diff(*this);
      diff-=w;
      return diff;
   ...
   __device__ Vec<N,T> operator*=(T a)
   {
      for(unsigned int i=0; i<N; ++i) v[i]*=a;
      return *this;
   }

   __device__ Vec<N,T> operator*(T a) const
   {
      Vec<N,T> w(*this);
      w*=a;
      return w;
...
template<unsigned int N, class T>
__device__ T mag2(const Vec<N,T> &a)
{
   T l=sqr(a.v[0]);
...
template<unsigned int N, class T>
__device__ T mag(const Vec<N,T> &a)
{ return sqrt(mag2(a)); }

template<unsigned int N, class T> 
__device__ inline T dist2(const Vec<N,T> &a, const Vec<N,T> &b)
{ 
   T d=sqr(a.v[0]-b.v[0]);
...
template<unsigned int N, class T> 
__device__ inline T dist(const Vec<N,T> &a, const Vec<N,T> &b)
{ return std::sqrt(dist2(a,b)); }

template<unsigned int N, class T> 
__device__ inline void normalize(Vec<N,T> &a)
{ a/=mag(a); }

template<unsigned int N, class T> 
__device__ inline Vec<N,T> normalized(const Vec<N,T> &a)
{ return a/mag(a); }

template<unsigned int N, class T> 
__device__ inline T infnorm(const Vec<N,T> &a)
{
   T d=std::fabs(a.v[0]);
...
template<unsigned int N, class T>
__device__ void zero(Vec<N,T> &a)
{ 
...
template<unsigned int N, class T> 
__device__ inline bool operator==(const Vec<N,T> &a, const Vec<N,T> &b)
{ 
   bool t = (a.v[0] == b.v[0]);
   unsigned int i=1;
   while(i<N && t) {
...
template<unsigned int N, class T>
__device__ inline Vec<N,T> operator*(T a, const Vec<N,T> &v)
{
   Vec<N,T> w(v);
   w*=a;
   return w;
...
template<unsigned int N, class T>
__device__ inline T min(const Vec<N,T> &a)
{
   T m=a.v[0];
...
template<unsigned int N, class T>
__device__ inline Vec<N,T> min_union(const Vec<N,T> &a, const Vec<N,T> &b)
{
   Vec<N,T> m;
...
template<unsigned int N, class T>
__device__ inline Vec<N,T> max_union(const Vec<N,T> &a, const Vec<N,T> &b)
{
   Vec<N,T> m;
   for(unsigned int i=0; i<N; ++i) (a.v[i] > b.v[i]) ? m.v[i]=a.v[i] : m.v[i]=b.v[i];
   return m;
...
template<unsigned int N, class T>
__device__ inline T max(const Vec<N,T> &a)
{
   T m=a.v[0];
   for(unsigned int i=1; i<N; ++i) if(a.v[i]>m) m=a.v[i];
   return m;
...
template<unsigned int N, class T>
__device__ inline T dot(const Vec<N,T> &a, const Vec<N,T> &b)
{
   T d=a.v[0]*b.v[0];
...
template<class T> 
__device__ inline Vec<2,T> rotate(const Vec<2,T>& a, float angle) 
{
   T c = cos(angle);
   T s = sin(angle);
...
template<class T>
__device__ inline Vec<2,T> perp(const Vec<2,T> &a)
{ return Vec<2,T>(-a.v[1], a.v[0]); } // counter-clockwise rotation by 90 degrees

template<class T>
__device__ inline T cross(const Vec<2,T> &a, const Vec<2,T> &b)
{ return a.v[0]*b.v[1]-a.v[1]*b.v[0]; }

template<class T>
__device__ inline Vec<3,T> cross(const Vec<3,T> &a, const Vec<3,T> &b)
{ return Vec<3,T>(a.v[1]*b.v[2]-a.v[2]*b.v[1], a.v[2]*b.v[0]-a.v[0]*b.v[2], a.v[0]*b.v[1]-a.v[1]*b.v[0]); }

template<class T>
__device__ inline T triple(const Vec<3,T> &a, const Vec<3,T> &b, const Vec<3,T> &c)
{ return a.v[0]*(b.v[1]*c.v[2]-b.v[2]*c.v[1])
        +a.v[1]*(b.v[2]*c.v[0]-b.v[0]*c.v[2])
        +a.v[2]*(b.v[0]*c.v[1]-b.v[1]*c.v[0]); }

template<unsigned int N, class T>
__device__ inline void assign(const Vec<N,T> &a, T &a0, T &a1)
{ 
   assert(N==2);
   a0=a.v[0]; a1=a.v[1];
}
...
template<unsigned int N, class T>
__device__ inline void assign(const Vec<N,T> &a, T &a0, T &a1, T &a2)
{ 
   assert(N==3);
...
template<unsigned int N, class T>
__device__ inline void assign(const Vec<N,T> &a, T &a0, T &a1, T &a2, T &a3)
{ 
   assert(N==4);
...
template<unsigned int N, class T>
__device__ inline void assign(const Vec<N,T> &a, T &a0, T &a1, T &a2, T &a3, T &a4, T &a5)
{ 
   assert(N==6);
...
template<unsigned int N, class T>
__device__ inline Vec<N,int> round(const Vec<N,T> &a)
{ 
   Vec<N,int> rounded;
   for(unsigned int i=0; i<N; ++i)
...
template<unsigned int N, class T>
__device__ inline Vec<N,int> floor(const Vec<N,T> &a)
{ 
   Vec<N,int> rounded;
   for(unsigned int i=0; i<N; ++i)
...
template<unsigned int N, class T>
__device__ inline Vec<N,int> ceil(const Vec<N,T> &a)
{ 
   Vec<N,int> rounded;
   for(unsigned int i=0; i<N; ++i)
...
template<unsigned int N, class T>
__device__ inline Vec<N,T> fabs(const Vec<N,T> &a)
{ 
   Vec<N,T> result;
   for(unsigned int i=0; i<N; ++i)
...
template<unsigned int N, class T>
__device__ inline void minmax(const Vec<N,T> &x0, const Vec<N,T> &x1, Vec<N,T> &xmin, Vec<N,T> &xmax)
{
   for(unsigned int i=0; i<N; ++i)
...
template<unsigned int N, class T>
__device__ inline void minmax(const Vec<N,T> &x0, const Vec<N,T> &x1, const Vec<N,T> &x2, Vec<N,T> &xmin, Vec<N,T> &xmax)
{
   for(unsigned int i=0; i<N; ++i)
...
template<unsigned int N, class T>
__device__ inline void minmax(const Vec<N,T> &x0, const Vec<N,T> &x1, const Vec<N,T> &x2, const Vec<N,T> &x3,
...
template<unsigned int N, class T>
__device__ inline void minmax(const Vec<N,T> &x0, const Vec<N,T> &x1, const Vec<N,T> &x2, const Vec<N,T> &x3, const Vec<N,T> &x4,
                   Vec<N,T> &xmin, Vec<N,T> &xmax)
{
   for(unsigned int i=0; i<N; ++i)
...
template<unsigned int N, class T>
__device__ inline void update_minmax(const Vec<N,T> &x, Vec<N,T> &xmin, Vec<N,T> &xmax)
{
